...
async fn test_command_building() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hget_cmd = HGetCommand::new("hash_key", "field");
    assert_eq!(hget_cmd.command_name(), "HGET");
...
async fn test_configuration_building() -> Result<(), Box<dyn std::error::Error>> {
    use redis_oxide::{ConnectionConfig, PoolConfig, PoolStrategy, ProtocolVersion, TopologyMode};
    ...
    let config = ConnectionConfig::new("redis://localhost:6379")
        .with_pool_config(pool_config.clone());
...
async fn test_resp3_value_types() -> Result<(), Box<dyn std::error::Error>> {
    use redis_oxide::protocol::resp3::Resp3Value;
...
async fn test_streams_types() -> Result<(), Box<dyn std::error::Error>> {
    use redis_oxide::streams::{StreamEntry, StreamRange, ReadOptions};
    use std::collections::HashMap;
...
async fn test_script_types() -> Result<(), Box<dyn std::error::Error>> {
    use redis_oxide::script::{Script, ScriptManager};
    ...
    let scripts = manager.list_scripts().await;
...
async fn test_sentinel_types() -> Result<(), Box<dyn std::error::Error>> {
    use redis_oxide::sentinel::{SentinelConfig, SentinelEndpoint, MasterInfo};
    use std::time::Duration;
