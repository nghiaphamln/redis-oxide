...
async fn create_test_client() -> Client {
    ...
    Client::connect(config).await.expect("Failed to connect to Redis")
}
...
async fn test_pipeline_basic_operations() {
    ...
    let _ = client.del(vec!["pipeline_test1".to_string(), "pipeline_test2".to_string()]).await;
...
async fn test_pipeline_exists_and_expire() {
    ...
    // Clean up any existing test keys
    let _ = client.del(vec!["pipeline_exists1".to_string(), "pipeline_exists2".to_string()]).await;
    ...
    pipeline.exists(vec!["pipeline_exists1".to_string(), "pipeline_exists2".to_string()]);
...
async fn test_pipeline_with_multiplexed_pool() {
    ...
    let client = Client::connect(config).await.expect("Failed to connect to Redis");
    
    // Clean up any existing test keys
    let _ = client.del(vec!["pipeline_mux1".to_string(), "pipeline_mux2".to_string()]).await;
...
async fn test_pipeline_with_connection_pool() {
    ...
    // Clean up any existing test keys
    let _ = client.del(vec!["pipeline_pool1".to_string(), "pipeline_pool2".to_string()]).await;
...
async fn test_pipeline_reuse() {
    ...
    // Clean up any existing test keys
    let _ = client.del(vec!["pipeline_reuse1".to_string(), "pipeline_reuse2".to_string()]).await;
    ...
    let results1 = pipeline.execute().await.expect("First pipeline execution failed");
    ...
    let results2 = pipeline.execute().await.expect("Second pipeline execution failed");
